...
export async function parseMiniFigAvgPrice({ miniFig: { condition, itemNumber }, page, index }: Params): Promise<string> {
  let avgPrice = "";
  ...
  try {
    ...
    const setTab = await page.waitForSelector("xpath/" + "//table[@id='_idTabMenu']/tbody/tr/td[contains(text(),'Minifigure')]");
    await setTab?.click();

    const item = await page.waitForSelector("xpath/" + "(//div[@id='_idItemTableForM']/table/tbody/tr)[2]/td[@class='pspItemClick']/a");
    await item?.click();
